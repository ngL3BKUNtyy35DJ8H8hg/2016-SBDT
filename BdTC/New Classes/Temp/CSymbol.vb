...
Namespace SaBan
    Public Class CSymbol
        ...
        Public Sub Draw(ByVal pMap As AxMap, ByVal g As Graphics)
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim dx As Single
            Dim dy As Single
        ...
        Public Sub DanhDau(ByVal pMap As AxMap, ByVal g As Graphics, ByVal Color As Color)
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim dx As Single
            Dim dy As Single
        ...
        Public Sub DanhDau(ByVal pMap As AxMap, ByVal g As Graphics, ByVal Pen As Pen)
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim dx As Single
            Dim dy As Single
        ...
        Public Sub DanhDau(ByVal pMap As AxMap, ByVal g As Graphics, ByVal pObj As GraphicObject, ByVal Color As Color)
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim dx As Single
            Dim dy As Single
        ...
        Public Function GetBounds(ByVal pMap As AxMap) As RectangleF
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
        ...
        Public Sub VeBound(ByVal pMap As AxMap, ByVal g As Graphics, ByVal Color As Color)
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim dx As Single
            Dim dy As Single
        ...
        Public Sub VeBound(ByVal pMap As AxMap, ByVal g As Graphics, ByVal pObj As GraphicObject, ByVal Color As Color)
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim dx As Single
            Dim dy As Single
        ...
        Public Sub DrawNodes(ByVal pMap As AxMap, ByVal g As Graphics)
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim dx As Single
            Dim dy As Single
            ...
            Try
                ...
                While enumerator.MoveNext()
                    ...
                    modBdTC.DrawNodes(g, points, 3.0F)
                End While
            Finally
                Dim enumerator As IEnumerator
                If TypeOf enumerator Is IDisposable Then
        ...
        Public Sub DrawRoot(ByVal pMap As AxMap, ByVal g As Graphics)
            Dim num As Single
            Dim num2 As Single
            pMap.ConvertCoord(num, num2, Me.m_GocX, Me.m_GocY, ConversionConstants.miMapToScreen)
            Dim pen As Pen = New Pen(Color.Black, 2.0F)
            ' The following expression was wrapped in a checked-expression
            ' The following expression was wrapped in a unchecked-expression
            ' The following expression was wrapped in a unchecked-expression
            Dim rect As System.Drawing.Rectangle = New System.Drawing.Rectangle(CInt(Math.Round(CDec((num - 3.0F)))), CInt(Math.Round(CDec((num2 - 3.0F)))), 7, 7)
            g.DrawRectangle(pen, rect)
            pen.Dispose()
        End Sub

        Public Function HitTest(ByVal pMap As AxMap, ByVal pt As PointF) As Boolean
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim num2 As Single
            Dim num3 As Single
            ...
            matrix.Scale(1.0F / num, 1.0F / num, MatrixOrder.Append)
        ...
        Public Function HitTest(ByVal pMap As AxMap, ByVal rect As RectangleF) As Boolean
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim num2 As Single
            Dim num3 As Single
        ...
        Public Function FindObjectAtPoint(ByVal pMap As AxMap, ByVal pt As PointF) As GraphicObject
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim num2 As Single
            Dim num3 As Single
        ...
        Public Sub MoveNodeTo(ByVal pMap As AxMap, ByVal ENode As CFOUNDNODE, ByVal pt As PointF)
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim num2 As Single
            Dim num3 As Single
            ...
            If pFoundNode.FoundObject.GetObjType() = OBJECTTYPE.Curve Or pFoundNode.FoundObject.GetObjType() = OBJECTTYPE.Line Then
                ...
                If pFoundNode.NodeIndex = 0 Or pFoundNode.NodeIndex = nodesShapeGraphic.Nodes.Count - 1 Then
                    ...
                    If cFOUNDNODE.NodeIndex = 0 Then
                        ...
                        result = True
                    ElseIf cFOUNDNODE.NodeIndex > 0 Then
            ...
            Try
                ...
                While enumerator.MoveNext()
                    ...
                    If (graphicObject.GetObjType() = OBJECTTYPE.Curve Or graphicObject.GetObjType() = OBJECTTYPE.Line) AndAlso graphicObject IsNot pFoundNode.FoundObject Then
                        ...
                        ElseIf nodesShapeGraphic.Nodes(nodesShapeGraphic.Nodes.Count - 1).X = nodesShapeGraphic2.Nodes(pFoundNode.NodeIndex).X And nodesShapeGraphic.Nodes(nodesShapeGraphic.Nodes.Count - 1).Y = nodesShapeGraphic2.Nodes(pFoundNode.NodeIndex).Y Then
                            cFOUNDNODE.FoundObject = graphicObject
        ...
        Public Function FindNodeAtPoint(ByVal pMap As AxMap, ByVal pt As PointF) As CFOUNDNODE
            Dim cFOUNDNODE As CFOUNDNODE = New CFOUNDNODE()
            Dim num As Single = CSng((Me.m_Zoom / pMap.Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, pMap.MapScreenWidth / Me.m_MWidth, 1)))
            Dim num2 As Single
            ...
            For i As Integer = Me.m_GObjs.Count - 1 To 0 Step -1
                Dim graphicObject As GraphicObject = Me.m_GObjs(i)
                Dim num4 As Integer = graphicObject.FindNodeAtPoint(pt2, 4.0F)
                If num4 > -1 Then
        ...
        Public Function RootHitTest(ByVal pMap As AxMap, ByVal pt As PointF) As Boolean
            Dim num As Single
            Dim num2 As Single
            pMap.ConvertCoord(num, num2, Me.m_GocX, Me.m_GocY, ConversionConstants.miMapToScreen)
            Dim rectangleF As RectangleF = New RectangleF(num - 3.0F, num2 - 3.0F, 7.0F, 7.0F)
            Return rectangleF.Contains(pt)
        End Function

        Public Sub Move(ByVal pMap As AxMap, ByVal fromPt As PointF, ByVal toPt As PointF)
            Dim num As Single
            Dim num2 As Single
            pMap.ConvertCoord(num, num2, Me.m_GocX, Me.m_GocY, ConversionConstants.miMapToScreen)
            num += toPt.X - fromPt.X
        ...
        Public Sub Shift(ByVal pMap As AxMap, ByVal deltaX As Single, ByVal deltaY As Single)
            Dim num As Single
            Dim num2 As Single
            pMap.ConvertCoord(num, num2, Me.m_GocX, Me.m_GocY, ConversionConstants.miMapToScreen)
            num += deltaX
        ...
        Public Sub Shift(ByVal deltaX As Double, ByVal deltaY As Double)
            Me.m_GocX += deltaX
            Me.m_GocY += deltaY
        End Sub

        Public Sub ChangeRoot(ByVal pMap As AxMap, ByVal newGocX As Double, ByVal newGocY As Double)
            Dim num As Single = CSng((pMap.Zoom / Me.m_Zoom))
            num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, Me.m_MWidth / pMap.MapScreenWidth, 1)))
            Dim num2 As Single
            Dim num3 As Single
            pMap.ConvertCoord(num2, num3, Me.m_GocX, Me.m_GocY, ConversionConstants.miMapToScreen)
            Dim num4 As Single
        ...
        Public Sub ChangeZoomMWidtht(ByVal newZoom As Double, ByVal newMWidth As Single)
            ...
            Try
                ...
                While enumerator.MoveNext()
                    ...
                    num = Conversions.ToSingle(Operators.MultiplyObject(num, Interaction.IIf(Me.m_MWidth > 0.0F, newMWidth / Me.m_MWidth, 1)))
                    Dim matrix As Matrix = New Matrix()
                    matrix.Reset()
        ...
        Public Sub Rotate(ByVal pMap As AxMap, ByVal rootPt As PointF, ByVal fromPt As PointF, ByVal toPt As PointF)
            Dim num As Single
            Dim num2 As Single
            pMap.ConvertCoord(num, num2, Me.m_GocX, Me.m_GocY, ConversionConstants.miMapToScreen)
            num -= rootPt.X
        ...
        Public Sub Rotate2(ByVal pMap As AxMap, ByVal rootPt As PointF, ByVal degree As Double)
            Dim num As Single
            Dim num2 As Single
            pMap.ConvertCoord(num, num2, Me.m_GocX, Me.m_GocY, ConversionConstants.miMapToScreen)
            num -= rootPt.X
        ...
        Public Sub Scale(ByVal pMap As AxMap, ByVal rootPt As PointF, ByVal fromPt As PointF, ByVal toPt As PointF)
            Dim num As Single
            Dim num2 As Single
            pMap.ConvertCoord(num, num2, Me.m_GocX, Me.m_GocY, ConversionConstants.miMapToScreen)
            num -= rootPt.X
        ...
        Public Sub Scale2(ByVal pMap As AxMap, ByVal rootPt As PointF, ByVal deltaX As Single, ByVal deltaY As Single)
            Dim num As Single
            Dim num2 As Single
            pMap.ConvertCoord(num, num2, Me.m_GocX, Me.m_GocY, ConversionConstants.miMapToScreen)
            num -= rootPt.X
        ...
        Public Sub VFlip()
            ...
            Finally
                ...
                End If
            End Try
            Dim pX As Single = (bounds.Left + bounds.Right) / 2.0F
            Try
        ...
        Public Sub New(ByVal pMap As AxMap, ByVal pGObjs As CGraphicObjs)
            Me.New()
            Dim num As Single = 0.0F
            Dim num2 As Single = 0.0F
            pMap.ConvertCoord(num, num2, Me.m_GocX, Me.m_GocY, ConversionConstants.miScreenToMap)
            Me.m_Zoom = pMap.Zoom
        ...
        Public Sub New(ByVal pMap As AxMap, ByVal pGObjs As CGraphicObjs, ByVal pZoom As Double, ByVal pMWidth As Single)
            Me.New()
            Dim num As Single = 0.0F
            Dim num2 As Single = 0.0F
            pMap.ConvertCoord(num, num2, Me.m_GocX, Me.m_GocY, ConversionConstants.miScreenToMap)
        ...
        Public Sub New(ByVal pMap As AxMap, ByVal pt0 As PointF, ByVal pGObjs As CGraphicObjs)
            Me.New()
        ...
        Public Sub New(ByVal pMap As AxMap, ByVal pt0 As PointF, ByVal pGObjs As CGraphicObjs, ByVal pMapScreenWidth As Single)
            Me.New()
            Dim x As Single = pt0.X
            Dim y As Single = pt0.Y
            pMap.ConvertCoord(x, y, Me.m_GocX, Me.m_GocY, ConversionConstants.miScreenToMap)
        ...
        Public Sub New(ByVal pMap As AxMap, ByVal pt0 As PointF, ByVal pGObjs As CGraphicObjs, ByVal pZoom As Double, ByVal pMWidth As Single)
            Me.New()
            Dim x As Single = pt0.X
            Dim y As Single = pt0.Y
            pMap.ConvertCoord(x, y, Me.m_GocX, Me.m_GocY, ConversionConstants.miScreenToMap)
