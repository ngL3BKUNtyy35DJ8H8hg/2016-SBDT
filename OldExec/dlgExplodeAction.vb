...
Namespace BdTC
	...
	Public Class dlgExplodeAction
		...
		Public Sub New()
			...
			dlgExplodeAction.__ENCList.Add(New WeakReference(Me))
			Me.Mode = "Update"
		...
		Private Sub UpdateAction()
			...
			If Not flag Then
				...
				If flag Then
					Me.ActionDef.position = mTargets(0)
				End If
			End If
		...
		Private Function isOK() As Boolean
			...
			Dim flag As Boolean = Operators.CompareString(Me.ActionDef.ActionType, "ExplodeTex", False) = 0
			If flag Then
				Dim flag2 As Boolean = Me.txtObjName.Text.Length > 0
				If flag2 Then
					kq = True
				End If
			Else
				Dim flag2 As Boolean = Operators.CompareString(Me.ActionDef.ActionType, "ExplodeModel", False) = 0
				If flag2 Then
					flag = (Me.txtObjName.Text.Length > 0)
					If flag Then
						kq = True
					End If
				Else
					...
					If flag2 Then
						Me.ActionDef.ActionType = "ExplodeTex"
						kq = True
					Else
						flag2 = (Operators.CompareString(Me.txtObjType.Text, "Model", False) = 0)
						If flag2 Then
							Me.ActionDef.ActionType = "ExplodeModel"
							kq = True
						Else
							flag2 = (Me.txtObjType.Text.Length = 0)
							If flag2 Then
								Dim mTargets As List(Of Vector3) = CActionDefs.ReadTargets(Me.txtPath.Text)
								flag2 = (mTargets.Count >= 1)
								If flag2 Then
									Me.ActionDef.ActionType = "Explosion"
									kq = True
								Else
									Interaction.MsgBox("Ch" & ChrW(432) & "a có V" & ChrW(7883) & " trí.", MsgBoxStyle.OkOnly, Nothing)
								End If
							Else
								Interaction.MsgBox("Ch" & ChrW(432) & "a có V" & ChrW(7883) & " trí.", MsgBoxStyle.OkOnly, Nothing)
							End If
		...
		Public Sub Show(pform As dlgActions, pAction As CActionDef, pMode As String)
			Me.Parentf = pform
			Me.ActionDef = CType(pAction, CExplodeDef)
			Me.Mode = pMode
